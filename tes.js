...
function ChatLog() {
  ...
  if (chatLines.length === 0) {
    ...
    buttons.forEach(button => {
      if (button.innerText.includes("Safe Indonesian")) {
        button.click();
      }
    });
  } else {
    chatLines.forEach(chatLine => {
      var { username, message } = getUsernameAndMessage(chatLine.textContent.trim());

      if (message.includes("Rejoined")) {
        return;
      }
      if (username === botName) {
        return;
      ...
      if (message === ChatLog.lastMessage) {
        return;
      }
      ChatLog.lastMessage = message;
      sendTextApi(message);
      console.log(message);

      if (message.includes("Closing outdated socket")) {
        let buttons = document.querySelectorAll('button[aria-haspopup="true"]');
        buttons.forEach(button => {
          if (button.innerText.includes("Safe Indonesian")) {
            button.click();
  ...
  if (chatLog && chatLog.style.display === 'none') {
    let localTab = document.querySelector('.chat-log-tab:nth-of-type(1)');
    let personalTab = document.querySelector('.chat-log-tab:nth-of-type(2)');
    if (activeTab === 'local') {
      localTab.click();
